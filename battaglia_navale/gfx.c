...
void gfx_line_width(int width ){
	XSetLineAttributes(gfx_display,gfx_gc,width,LineSolid,CapButt,JoinMiter);
}

void gfx_draw_text(int x,int y,char *txt,int l){
	font = XLoadQueryFont(gfx_display,"-misc-fixed-medium-r-normal--18-*-*-*-*-*-iso8859-15");
	if(!font){
		printf("Caricamento font fallito");
		font = XLoadQueryFont(gfx_display,"fixed");
	}
	ti[0].chars = txt;
	ti[0].nchars = l;
	ti[0].delta = 0;
	ti[0].font = font->fid;
	XDrawText(gfx_display,gfx_window,gfx_gc,x,y,ti,1);
	XUnloadFont(gfx_display,font->fid);
}

void gfx_draw_ellipse(int x,int y, int with,int height){
	XFillArc(gfx_display,gfx_window,gfx_gc,x,y, with,height,360*64,360*64);
}
